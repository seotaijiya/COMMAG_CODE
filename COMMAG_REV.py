...
def random_power(channel, tx_max, granuty, noise, DUE_thr, I_thr, P_c, tx_power_set):
    num_sample = channel.shape[0]
    num_channel = channel.shape[1]
    ## Note that the num_user i
    num_D2D_user = channel.shape[2] - 1
    ...
    for i in range(num_sample):
        ...
        for j in range(num_channel):
            ...
            inter = cal_CUE_INTER_one_sample_one_channel(cur_ch, tx_power_select[:, j])
            cur_cap = cur_cap + cur_ch_cap
            CUE_mask = CUE_mask * (inter[num_D2D_user] < I_thr)
            if cur_ch_cap[num_D2D_user] < I_thr:
        ...
        summed_tx_power = np.sum(tx_power_select[:, :], axis=1) + P_c
        SE_sum = np.sum(cur_cap[:-1])
        EE_sum = np.sum(cur_cap[:-1]/summed_tx_power[:-1])
        PW_sum = np.sum(np.sum(tx_power_select[:, :], axis=1)[:-1])
...
for outer_loop in range(5):
    ...
    lambda_mat = np.ones((3, 1)) * 40
    lambda_mat[0] = 1
    lambda_mat[2] = lambda_mat[2] * 0.2


    lambda_ee_mat = np.ones((3, 1)) * 100
    lambda_ee_mat[0] = 1
    lambda_ee_mat[2] = lambda_ee_mat[2] * 0.4

    lambda_pw_mat = np.ones((3, 1)) * 10.0
    lambda_pw_mat[0] = 0.1
    lambda_pw_mat[1] = 800.0
    ...
    print("train_set result")
    _, _, _ = Print_Test_Full(model_SE, model_EE, model_PW, model_SE_super, model_EE_super,
                                             model_PW_super, log_data, data_train, Num_power_level, tx_max, noise,
                                             DUE_thr, I_thr, P_c, tx_power_set, SE_OPT, EE_OPT, PW_OPT)
